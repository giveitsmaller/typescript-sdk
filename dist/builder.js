...
export class OperationBuilder {
     ...
     */
    _refusePlannedValues(wireOptions) {
        const planned = _firstPlannedValue(this.opType, wireOptions);
        if (planned !== undefined) {
            throw new GislConfigError(`${this.opType}: '${planned.key}: ${String(planned.value)}' is advertised but not available yet (planned).`, { reason: 'feature_not_available', conflictingFields: [planned.key] });
        }
    }
    async run(options = {}) {
 ...
 * @internal
 */
export async function _retryOn429(call, args) {
    ...
    for (let attempt = 0;; attempt++) {
        try {
            ...
            const waitMs = retryAfterMs ?? Math.min(10_000, Math.floor(Math.random() * base * Math.pow(2, attempt)) + Math.min(250, base));
            if (Date.now() + waitMs >= args.deadline) {
                throw new GislTimeoutError(`Workflow ${args.workflowId}: rate limited (429) and the server's wait of ${waitMs} ms ` +
                    'ends past maxWait', args.workflowId);
            }
