 ...
 */
const AUTO_QUALITY_INCOMPATIBLE_KEYS = ['quality', 'lossless', 'target_size_bytes'];
/**
    ...
    lowerOutputStep(step, stepIndex) {
        ...
        if (wireOptions.quality_preset !== undefined) {
            if (wireOptions.encoding_mode === undefined) {
                ...
                throw new GislConfigError(`output(): 'quality_preset' requires encoding_mode 'auto_quality' (its contract dependency), ` +
                    `but got '${String(wireOptions.encoding_mode)}'. Omit encoding_mode to let quality_preset drive it, ` +
                    'or drop quality_preset.', { reason: 'invalid_option_combination', conflictingFields: ['quality_preset', 'encoding_mode'] });
            }
        }
        ...
        if (wireOptions.encoding_mode === 'auto_quality') {
            for (const field of AUTO_QUALITY_INCOMPATIBLE_KEYS) {
                if (wireOptions[field] !== undefined) {
                    throw new GislConfigError(`output(): '${field}' cannot be combined with encoding_mode 'auto_quality' — '${field}' ` +
                        `requires a different encoding_mode (its contract dependency), and auto_quality drives ` +
                        `the quality from quality_preset alone. Drop '${field}', or drop quality_preset/auto_quality.`, { reason: 'invalid_option_combination', conflictingFields: ['encoding_mode', field] });
                }
            }
        }
